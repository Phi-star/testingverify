...
// Per docs: GET /api/public/number_by_token?order_uuid=<ORDER_UUID>
// Query param name is 'order_uuid' per GlobalSMS API docs
// FIX: was incorrectly using 'token' as the param name — docs specify 'order_uuid'
app.get('/proxy/order-numbers', async (req, res) => {
  ...
  try {
    const url = `${BASE}/api/public/number_by_token?order_uuid=${encodeURIComponent(order_uuid)}`;
    const response = await fetch(url, {
      headers: { 'Authorization': req.headers.authorization, 'accept': '*/*' }
    });

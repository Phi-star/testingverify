...
// Docs example URL uses ?token= despite the table saying order_uuid.
// Using ?token= (no encoding — UUIDs are already URL-safe).
app.get('/proxy/order-numbers', async (req, res) => {
  ...
  try {
    const url = `${BASE}/api/public/number_by_token?token=${order_uuid}`;
    const response = await fetch(url, {
      headers: { 'Authorization': req.headers.authorization, 'accept': '*/*' }
    });
